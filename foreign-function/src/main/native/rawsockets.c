...
int socket_tester(const char *srcIp, const char *dstIp, int dstPort, int readTimeoutMs, int writeTimeoutMs);
...
void create_syn_packet(
    ...
    printf("[INFO (native)] TCP header sequence number: %u\n", ntohl(tcph->seq));
...
int receive_from(
    ...
    do {
        ...
        if (diffTime >= readTimeoutMs) {
            printf("[INFO (native)] Read timeout after %d > %d ms\n", diffTime, readTimeoutMs);
            return -2;
        }
    } while (dst_port != dst->sin_port);

    printf("[INFO (native)] Received bytes: %d\n", received);
    printf("[INFO (native)] Destination port: %d\n", ntohs(dst->sin_port));
...
int socket_tester(
    ...
    if (sock == -1) {
        printf("[ERROR (native)] Socket creation failed, you should be root\n");
        return FAIL;
    }
    struct timeval tv;
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (const char *) &tv, sizeof tv) == -1) {
        printf("[INFO (native)] Setsockopt(SO_RCVTIMEO, %d + %d) failed: %s\n", (int) tv.tv_sec, (int) tv.tv_usec,
               strerror(errno));
        return FAIL;
    }
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, (const char *) &tv, sizeof tv) == -1) {
        printf("[INFO (native)] Setsockopt(SO_SNDTIMEO,  %d + %d) failed: %s\n", (int) tv.tv_sec, (int) tv.tv_usec,
               strerror(errno));
        return FAIL;
    }
    ...
    if (inet_pton(AF_INET, dstIp, &daddr.sin_addr) != 1) {
        printf("[ERROR (native)] Destination IP configuration failed\n");
        return FAIL;
    }
    ...
    if (inet_pton(AF_INET, srcIp, &saddr.sin_addr) != 1) {
        printf("[ERROR (native)] Source IP configuration failed\n");
        return FAIL;
    }
    printf("[INFO (native)] Selected source port number: %d\n", ntohs(saddr.sin_port));
    ...
    if (setsockopt(sock, IPPROTO_IP, IP_HDRINCL, val, sizeof(one)) == -1) {
        printf("[ERROR (native)] Setsockopt(IP_HDRINCL, 1) failed\n");
        return FAIL;
    }
    // send SYN
    char *packet;
    ...
    if (sent == -1) {
        printf("[ERROR (native)] Sendto() failed\n");
        return FAIL;
    } else {
        printf("[INFO (native)] Successfully sent %d bytes SYN!\n", sent);
    }
    // receive SYN-ACK
    ...
    if (received == -1) {
        printf("[ERROR (native)] Receive_from() failed\n");
        return FAIL;
    } else if (received == -2) {
        printf("[INFO (native)] Receive_from() timeout\n");
        return TIMEOUT;
    } else {
        printf("[INFO (native)] Successfully received %d bytes\n", received);

        struct iphdr *iph = (struct iphdr *) recvbuf;
        struct tcphdr *tcph = (struct tcphdr *) (recvbuf + sizeof(struct iphdr));

        printf("[INFO (native)] Received syn: %d, ack: %d, rst: %d\n", tcph->syn, tcph->ack, tcph->rst);
